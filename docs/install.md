...
Please see details on installing `scanpy` and its dependencies [here](https://scanpy.readthedocs.io/en/stable/installation.html).
...
For hacking on the package, it is most convenient to do a so-called development-mode install, which symlinks files in your Python package directory to your muon working directory, such that you do not need to reinstall after every change.
We use [flit](https://flit.readthedocs.io/en/latest/index.html) as our build system.
After installing flit, you can run `flit install -s` from within the muon project directory to perform a development-mode install.
Happy hacking!

...
Using the slicing syntax (e.g. `adata[cell_ids]`) results in a `View` object, which has to be copied then for any modidying operations.
While this behaviour can be useful in many cases, that nearly doubles the amount of required memory and introduces unnecessary challenges when handling exceptionally large datasets.

`muon` introduces functions for in-place filtering: {func}`muon.pp.filter_obs` and {func}`muon.pp.filter_var`.
These function directly modify the `AnnData` object that they are called on.
Their syntax allows them to also be more general than the aforementioned filtering functions.
...
In-place filtering is not defined for views.
On backed objects, it has to be used with care: data for the requested subset of `.X` will be read into memory and the object will not be backed anymore.
...
It can also be called on the {class}`~mudata.MuData` object in the same way.

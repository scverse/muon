...
def embedding(
    ...
    **kwargs,
    ...
    if mod not in data.mod:
        raise ValueError(f"Modality {mod} is not present in the MuData object with modalities {', '.join(data.mod)}")
    ...
    if basis_mod not in adata.obsm:
        if 'X_'+basis_mod in adata.obsm:
            ...
            raise ValueError(f"Basis {basis_mod} is not present in the modality {mod} with embeddings {', '.join(adata.obsm)}")
        else:
        ...
        for m in mdata.mod:
            if np.sum(keys_in_mod[m]) > 0:
                ...
                if use_raw is None or use_raw:
                    if mdata.mod[m].raw is not None:
                        fmod_adata = mdata.mod[m].raw[:,mod_keys]
                    else:
                        if use_raw:
                            warnings.warn(f"Attibute .raw is None for the modality {m}, using .X instead")
                        fmod_adata = mdata.mod[m][:,mod_keys]
                else:
                    fmod_adata = mdata.mod[m][:,mod_keys]

                if layer is not None:
                    if layer in mdata.mod[m].layers:
                        fmod_adata.X = mdata.mod[m][:,mod_keys].layers[layer].X
                        if use_raw:
                ...
                obs = obs.join(pd.DataFrame(fmod_adata.X, columns=mod_keys, index=fmod_adata.obs_names), how="left")

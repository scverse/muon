...
def mofa(
    ...
    if isinstance(data, MuData):
        ...
        if len(common_obs) != mdata.n_obs:
            if not use_obs:
                ...
                raise ValueError(f"Expected `use_obs` argument to be 'union' or 'intersection', not '{use_obs}'")
        else:
            use_obs = None
    ...
    if "warping_ref" in smooth_kwargs:
        ...
        if not (isinstance("warping_ref", int)):
            ...
            smooth_kwargs["warping_ref"] = warping_ref[0]  # type: ignore[index]
    ...
    if smooth_kwargs is not None and "new_values" in smooth_kwargs and smooth_kwargs["new_values"] and smooth_covariate:
    ...
    try:
        ...
        warn("Cannot save variance estimates", stacklevel=2)
...
def snf(
    ...
    if neighbor_keys is None:
        ...
        modalities = mdata.mod.keys()
        neighbor_keys = dict.fromkeys(modalities, neighbor_keys)
    else:
    ...
    logging.info(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Starting {n_iterations} iterations...")
    for ti in range(n_iterations):
        ...
        logging.info(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Done: iteration {ti} of {n_iterations}.")
    ...
    **kwargs,
    ...
    if resolution:
        if isinstance(resolution, Mapping):
            ...
            parts = [partition_type(gs[mod], resolution_parameter=resolution, **partition_kwargs) for mod in mdata.mod]
    else:
...
def umap(
    ...
    try:
        ...
        raise ValueError(f'Did not find .uns["{neighbors_key}"]. Run `muon.pp.neighbors` first.') from None

...
def dsb(
    ...
    if data_raw is None:
        if empty_counts_range is None or cell_counts_range is None:
            raise ValueError("data_raw is None, assuming data is the unfiltered object, but no count ranges provided")
        ...
        empty_idx = np.where((log10umi >= min(*empty_counts_range)) & (log10umi < max(*empty_counts_range)))[0]
        cell_idx = np.where((log10umi >= min(*cell_counts_range)) & (log10umi < max(*cell_counts_range)))[0]
    ...
    if data_raw is not None and cell_counts_range is not None:
        warn("cell_counts_range values are ignored since cells are provided in data", stacklevel=2)

    empty_scaled = np.log(empty.X + pseudocount) if not issparse(empty.X) else np.log(empty.X.toarray() + pseudocount)
    cells_scaled = np.log(cells.X + pseudocount) if not issparse(cells.X) else np.log(cells.X.toarray() + pseudocount)
    ...
    if denoise_counts:
        ...
        for c in range(cells_scaled.shape[0]):
            ...
            if sharedvar.bic(cells_scaled[c, :, np.newaxis]) < separatevar.bic(cells_scaled[c, :, np.newaxis]):
        ...
        if isotype_controls is not None:
            ...
            if len(ctrl_idx) < len(isotype_controls):
                warn("Some isotype controls are not present in the data.", stacklevel=2)
            covar = pca.fit_transform(np.hstack((cells_scaled[:, ctrl_idx], bgmeans.reshape(-1, 1))))
        else:
    ...
    if flavor == "seurat":
        if issparse(x):
            if axis == 0 and not isinstance(x, csc_matrix | csc_array):
                ...
                x = x.tocsc()
            elif axis == 1 and not isinstance(x, csr_matrix | csr_array):
                warn(

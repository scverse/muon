...
def read_snap(filename: PathLike, matrix: str, bin_size: int | None = None):
    ...
    if bin_size is not None:
        if matrix not in ["bm", "bins"]:
            warn("Argument bin_size is only relevant for bins matrix (BM) and will be ignored", stacklevel=2)
    ...
    if matrix == "pm" or matrix == "peaks":
        if "PM" in f:
            ...
            features = (
                np.char.array(chrom) + ":" + np.char.array(start).astype("str") + "-" + np.char.array(end).astype("str")
            )
